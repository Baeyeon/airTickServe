...
public class queryTransferServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        List<Object[]> list = airlineService.queryTransfer(airline);
//        Airline[] airlines = null;
        List<Airline> airlines = new ArrayList();;
        for( Object[] objs  : list){
            for(Object obj : objs){
                 airlines.add(airlineService.queryAirlineById(obj.toString()));
            }
        }
        Airline[] airlinesArr = airlines.toArray(new Airline[airlines.size()]);
        out.print("[");
        for(Airline airlineObj : airlinesArr){
            out.print(airlineObj.toString()+",");
        }
        out.print("]");

//        Airline[] list = null;
//        list[0] = airlineService.queryAirlineById(A_id);
//        list[1] = airlineService.queryAirlineById(B_id);
//        out.print("["+airlineService.queryAirlineById(A_id)+","+airlineService.queryAirlineById(B_id)+"]");
    }
}
